...
class PriorityQueue:
    ...
    def append(self, key):
        ...
        # if queue is empty place item at root

        if len(self.queue) == 0:
            self.queue.append(key)

        # else place it at bottom of heap and swap with parent if required
        else:
            self.queue.append(key)
            k = len(self.queue)

            p = k // 2
            parent = self.queue[p - 1]

            while (key < parent):
                self.queue[k - 1] = parent
                self.queue[p - 1] = key
                k = p
                p = k // 2
    ...
    def min(self):
        ...
        return self.queue[self.min_index - 1]
    ...
    def pop(self):
        ...
        # swap heap's root with last element
        self.queue[0] = self.queue[-1]
        self.queue[-1] = min_elt
    ...
    def _min_heapify(self, index):
        ...
        if (l <= len(self.queue)) and (self.queue[l-1] < self.queue[index-1]):
            smallest = l
        else:
            smallest = index
        if (r <= len(self.queue)) and (self.queue[r-1] < self.queue[smallest-1]):
            smallest = r
        if smallest != index:
            swap_var = self.queue[index - 1]
            self.queue[index-1] = self.queue[smallest-1]
            self.queue[smallest-1] = swap_var
